...
protocol RowHolder {
    associatedtype RowType: DeepHashable & Hashable
    func getRows() -> [RowType]
}

class LegacyBatchUpdater: BatchUpdater {
    public func batchUpdate(tableView: UITableView,
        ...
        for updateInfo in changes.rowsChanges.updates {
            guard let oldIndexPath = updateInfo.old,
                let index = (tableView.indexPathsForVisibleRows ?? []).firstIndex(of: oldIndexPath) else {
    ...
    public func batchUpdate(collectionView: UICollectionView,
    ...
    func changes(forOldSection oldSections: [BatchUpdateSection],
                 and newSections: [BatchUpdateSection]) -> (sectionsChanges: SectionsChanges,
        ...
        for (index, section) in newSections.enumerated() {
            ...
            guard let oldSectionInfo = oldSections.enumerated()
                .first(where: { $0.element.deepDiffHash == section.deepDiffHash }) else { continue }
    ...
    private func diffSet(oldList: [DeepHashable], list: [DeepHashable]) -> SectionsChanges {
        ...
        for changeValue in diffResult {
            ...
            case let .move(move):
                guard let fromIndex = oldList.firstIndex(where: { $0.deepDiffHash == move.item.deepDiffHash }),
                    let toIndex = list.firstIndex(where: { $0.deepDiffHash == move.item.deepDiffHash })
                else { continue }
                guard fromIndex != toIndex else { continue }
                sectionsChanges.moves.append(SectionsChanges.Move(from: fromIndex, to: toIndex))

            case let .replace(repl):
                guard let newIndex = list.firstIndex(where: { $0.deepDiffHash == repl.newItem.deepDiffHash })
                else { continue }
        ...
        for newListIndex in 0 ..< list.count {
            if filteredUpdates.contains(newListIndex) {
                ...
                guard let oldListIndex = oldList.firstIndex(where: {
                    $0.deepDiffHash == list[newListIndex].deepDiffHash
                }) else { continue }
    ...
    private func diffIndexPath<T: DeepHashable>(oldList: [T],
        ...
        for changeValue in diffResult {
            ...
            case let .move(move):
                rowsChanges.moves.append(RowsChanges.Move(from: IndexPath(row: move.fromIndex,
                                                                          section: oldSectionIndex),
                                                          to: IndexPath(row: move.toIndex,
